module.exports = (schema, typeReq) => {
  ...
  if (typeReq !== 'updatePriceDates') {
    ...
    options = [
      {
        key: keys[0].key, // updatePriceDates
        title: keys[0].schema.$_terms.items[0]._flags.description,
        type: keys[0].schema.$_terms.items[0].$_terms.notes[0],
        placeholder: keys[0].schema.$_terms.items[0].$_terms.examples[0],
        validation: {
          length: keys[0].schema.$_terms.items[0]._rules[0].args.limit,
          warningMessages: {
            length:
              keys[0].schema.$_terms.items[0]._preferences.messages[
                'string.length'
              ]._template[0] +
              keys[0].schema.$_terms.items[0]._rules[0].args.limit +
              keys[0].schema.$_terms.items[0]._preferences.messages[
                'string.length'
              ]._template[
                keys[0].schema.$_terms.items[0]._preferences.messages[
